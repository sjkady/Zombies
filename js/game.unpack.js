    ...
    if (this.health >= 0) {
      ...
      if (Game.controls.up) {
        this.prey += this.speed * Math.cos(this.direction * Math.PI / 180) * step;
		      ...
		      if (Math.abs(this.prex - obstacles[k].x) <= obstacles[k].width / 2 + this.width / 2 && Math.abs(this.prey - obstacles[k].y) <= obstacles[k].height / 2 + this.height) {
		        this.movey = false;
		        this.prey -= this.speed * Math.cos(this.direction * Math.PI / 180) * step;
		      }
		    }
        this.prex += this.speed * Math.sin(this.direction * Math.PI / 180) * step;
				for (var t = obstacles.length - 1; t >= 0; t--) {
					if (Math.abs(this.prex - obstacles[t].x) <= obstacles[t].width / 2 + this.width / 2 && Math.abs(this.prey - obstacles[t].y) <= obstacles[t].height / 2 + this.height) {
						this.movex = false;
						this.prex -= this.speed * Math.sin(this.direction * Math.PI / 180) * step;
					}
				}
      ...
      if (Game.controls.down) {
        this.prey -= this.speed * Math.cos(this.direction * Math.PI / 180) * step;
		      ...
		      if (Math.abs(this.prex - obstacles[j].x) <= obstacles[j].width / 2 + this.width / 2 && Math.abs(this.prey - obstacles[j].y) <= obstacles[j].height / 2 + this.height) {
		        this.movey = false;
		        this.prey += this.speed * Math.cos(this.direction * Math.PI / 180) * step;
		      }
		    }
        this.prex -= this.speed * Math.sin(this.direction * Math.PI / 180) * step;
				for (var v = obstacles.length - 1; v >= 0; v--) {
					if (Math.abs(this.prex - obstacles[v].x) <= obstacles[v].width / 2 + this.width / 2 && Math.abs(this.prey - obstacles[v].y) <= obstacles[v].height / 2 + this.height) {
						this.movex = false;
						this.prex += this.speed * Math.sin(this.direction * Math.PI / 180) * step;
					}
				}
...
(function() {
  ...
  Zombie.prototype.update = function(step, player, obstacles, zombies, index) {
    ...
    for (var h = zombies.length - 1; h >= 0; h--) {
      if (h != index) {
        if (Math.abs(this.prex - zombies[h].x) < this.width / 2 + zombies[h].width / 2 && Math.abs(this.prey - zombies[h].y) < this.height / 2 + zombies[h].height) {
          this.movex = false;
          this.prex -= this.dx / this.mag * this.speed * step;
        }
      }
    ...
    for (var l = zombies.length - 1; l >= 0; l--) {
      if (l != index) {
        if (Math.abs(this.prex - zombies[l].x) < this.width / 2 + zombies[l].width / 2 && Math.abs(this.prey - zombies[l].y) < this.height / 2 + zombies[l].height) {
          this.movex = false;
          this.prey -= this.dy / this.mag * this.speed * step;
        }
      }
...
//Corpse
(function() {
  function Corpse(shotx, shoty, zombiex, zombiey, rotation) {
    this.shotx = shotx;
    this.shoty = shoty;
  ...
  Corpse.prototype.draw = function(context, xView, yView) {
		context.save();

    context.translate((this.zombiex - xView), (this.zombiey - yView));
    context.rotate(-(this.rotation));
    context.translate(-(this.zombiex - xView), -(this.zombiey - yView));
		context.globalAlpha = this.hit;
		context.fillStyle = '#8A0707';
		context.strokeStyle = '#8F1515';
		context.roundRect(this.zombiex - this.width / 2 - xView, this.zombiey - this.height / 2 - yView, this.width, this.height, 5, 5, true,true );
		context.globalAlpha = this.opacity;
    context.fillStyle = '#5E932D';
		context.strokeStyle = '#5a632D';
    context.roundRect(this.zombiex - this.width / 2 - xView, this.zombiey - this.height / 2 - yView, this.width, this.height, 5, 5, true,true );
	  context.beginPath();
    context.arc(this.zombiex - xView, this.zombiey - yView + 3, this.height * 0.45, 0, 2 * Math.PI, false);
    context.fillStyle = '#A2AD59';
    context.fill();
    context.lineWidth = 5;
    context.strokeStyle = '#B8AD59';
    ...
    context.globalAlpha = this.opacity;
		context.beginPath();
		context.arc(this.shotx - xView, this.shoty - yView + 3, this.age, 0, 2 * Math.PI, false);
		context.fillStyle = '#8A0707';
		context.fill();
		context.lineWidth = 5;
		context.strokeStyle = '#8F1515';
...
// Gun
(function() {
  function Gun(name, damage, ammo, clip, splash, piercing, cooldown, reload, speed, width, height, cost, owned) {
    this.name = name;
    this.damage = damage;
    this.ammo = ammo;
    this.clip = clip;
...
(function() {
  ...
  GunBuy.prototype.draw = function(context, xView, yView) {
    ...
    context.restore();
		...
		context.fillText(this.gun.name, this.x - this.width / 2 - xView, this.y - yView - 10);
		context.fillText(this.gun.cost, this.x - this.width / 2 - xView, this.y - yView + 10);
		context.restore();
  };
  Game.GunBuy = GunBuy;
}());
...
(function() {
  ...
  Map.prototype.generate = function() {
    ...
    var rows = Math.floor(this.width / 10) + 1;
    var columns = Math.floor(this.height / 2) + 1;
    ctx.save();
...
(function() {
  ...
  var guns = [ /*name,		damage,	ammo,	clip,	splash,	pierce	cd, 	rd, sp,	 w, h, cost, O*/
    new Game.Gun('Base Pistol', 1, 64, 8, 0, 0, 1, 2, 700, 4, 4, 500, 1),
    new Game.Gun('Semi Pistol', 0.7, 120, 3, 0, 0, 0.03, 2, 750, 3, 3, 800,0),
    new Game.Gun('Machine Gun', 1, 300, 30, 0, 0, 0.2, 5, 750, 4, 4, 4000,0),
    new Game.Gun('Pea Shooter', 0.5, 200, 20, 0, 0, 0.1, 3, 800, 2, 2, 1500,0),
    new Game.Gun('Rifle', 5, 50, 5, 0, 0, 3, 5, 700, 6, 6, 2000,0)
  ];
  //give defualt pistol
  ...
  var roundup = function() {
    ...
    gunbuy.pop();
    gunbuy.push(new Game.GunBuy(Math.floor(Math.random() * room.width), Math.floor(Math.random() * room.height), guns[Math.floor(Math.random() * guns.length)]));
  };
        ...
        if (typeof shots[i] != 'undefined') {
          if (Math.abs(zombies[g].x - shots[i].x) < zombies[g].width / 2 + shots[i].width / 2 && Math.abs(zombies[g].y - shots[i].y) < zombies[g].height / 2 + shots[i].height / 2) {
            ...
            if (zombies[g].health <= 0) {
							corpses.push(new Game.Corpse(shots[i].x, shots[i].y, zombies[g].x, zombies[g].y, zombies[g].rotation));
							zombies.splice(g, 1);
              player.kills += 1;
              player.points += 50;
  ...
  var draw = function() {
    ...
    for (var i = zombies.length - 1; i >= 0; i--) {
      zombies[i].draw(context, camera.xView, camera.yView);
		...
		for (var v = corpses.length - 1; v >= 0; v--) {
			corpses[v].draw(context, camera.xView, camera.yView);
		}
